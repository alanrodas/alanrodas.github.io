...
function autoload_i18n(lang) {
	jQuery.i18n.properties({
		name: 'i18n/messages',
		language: lang,
		callback: function(){
			$("[data-i18n-property]").each(function(i, each){
				var message = $(each).attr("data-i18n-property");
				if ($(each).attr("data-i18n-target") === "text" || !$(each).attr("data-i18n-target")) {
					$(each).text(eval(message))
				} else {
					$(each).attr($(each).attr("data-i18n-target"), eval(message))
				}
			});
		}
	});
...
jQuery(document).ready(function ($) {
    $("[data-toggle=switch-lang]").each(function(i, each){
		$(each).click(function(event){
			event.preventDefault();
			autoload_i18n($(each).attr("data-i18n-lang"))
		});
	});
    autoload_i18n();
}(jQuery))